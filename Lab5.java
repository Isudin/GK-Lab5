...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		if (objectNumber == 1)
		{
			Cork(gl2, versionNumber);
		}
		else if (objectNumber == 2)
		{
			Pyramid(gl2, versionNumber);
		}
	}
	
	public void Cork(GL2 gl2, int numOfSpins) 
	{
		double size = Math.PI * numOfSpins;
		y = 20;

		gl2.glBegin(GL2.GL_LINE_STRIP);

		for (float i = 0; i <= size * 2; i += 0.2) {
			x = (float) (Math.sin(i) * (3-i/50));
			z = (float) (Math.cos(i) * (3-i/50));
			y -= 0.09;

			gl2.glColor3f(0.0f, 1.0f, 0.0f);
			gl2.glVertex3f(x/2, y/4, z/2);
		}
		gl2.glEnd();
	}
	
	
	public void Pyramid(GL2 gl2, float numOfStages) 
	{
		y = -5;
		gl2.glColor3f(0.0f, 1.0f, 0.0f);
		...
		gl2.glBegin(gl2.GL_POLYGON);
			...
			{
				x = (float) (Math.cos(i * 2 * Math.PI / numOfStages) * 4);
				z = (float) (Math.sin(i * 2 * Math.PI / numOfStages) * 4);
				gl2.glVertex3f(x, (float) (y-0.01), z);
			}
		gl2.glEnd();
		...
		for (int i = 0; i < numOfStages; i++) {
			x = (float) (Math.sin(i * 2 * Math.PI / numOfStages) * 4);
			z = (float) (Math.cos(i * 2 * Math.PI / numOfStages) * 4);
